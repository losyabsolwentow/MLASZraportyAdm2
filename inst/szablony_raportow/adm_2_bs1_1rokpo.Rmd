---
title: |
  | Raport roczny z monitoringu karier absolwentów, którzy ukończyli branżową szkołę I stopnia w roku szkolnym `r params$rok - 1`/`r params$rok`
subtitle: |
...
`r ifelse(szk$l_abs[[1]] >= progLiczebnosci, paste0("W raporcie przedstawiono sytuację absolwentów szkoły z rocznika ", rocznik, " w ", params$rok, " roku. "), "")`Przypisanie absolwentowi określonego statusu (np. “uczy się” lub “pracuje”) w danym miesiącu oznacza, że przynajmniej przez jeden dzień w tym miesiącu miał taki status. 
...
Żeby zwiększyć przydatność raportów, po rozważaniu uwag zgłoszonych przez dyrektorów szkół, nauczycieli i doradców zawodowych, wprowadziliśmy zmiany w stosunku do pierwszej edycji. Polegają one przede wszystkim na:

* zastosowaniu jako grupy odniesienia absolwentów z powiatu, podregionu, województwa lub kraju (zgodnie z zasadą opisaną w poprzednim podrozdziale),
...
* uzupełnieniu raportów o informacje na temat branż, w których podjęli naukę w branżowych szkołach II stopnia absolwenci wykształceni w poszczególnych zawodach (o ile liczebność grup wyróżnionych ze względu na wyuczony zawód na to pozwala),

* uproszczeniu sposobu prezentacji sytuacji absolwentów poprzez zmniejszenie liczby statusów odzwierciedlających ich sytuację edukacyjną i zawodową z sześciu do czterech; kategoria “uczy się i brak informacji o sytuacji zawodowej” została połączona z kategorią “uczy się i nie pracuje na podstawie umowy o pracę lub samozatrudnienia”, a kategoria “nie uczy się i brak informacji o sytuacji zawodowej” z kategorią “nie uczy się i nie pracuje”,

* rezygnacji z prezentowania kwot miesięcznych przychodów z pracy, ze względu na brak informacji o wymiarze zatrudnienia absolwentów w poszczególnych miesiącach.

Instytut Badań Edukacyjnych pragnie podziękować uczestnikom spotkań poświęconych pierwszej edycji raportów oraz osobom, które wypełniły ankietę dotyczącą ich zakresu i możliwości wykorzystania. Zgłoszone uwagi posłużyły do ulepszenia drugiej edycji raportów i zostaną wykorzystane w pracach nad kolejnymi. Staramy się, żeby relacja pomiędzy zaangażowaniem potrzebnym do zapoznania się z raportem a przydatnością przedstawionych w nim wyników była dla Państwa jak najkorzystniejsza.
...
Prosimy o wypełnienie ankiety do końca czerwca 2023 roku.

## Wykorzystanie raportu

Przy wykorzystywaniu informacji z raportów prosimy o podanie źródła:

*Raport roczny z monitoringu karier absolwentów, którzy ukończyli `r szk$dane_szkoly[[1]]$nazwa` w `r params$rok` roku, prowadzonego na podstawie danych administracyjnych. Warszawa: Instytut Badań Edukacyjnych, 2022.*
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  ...
  cat("Liczba absolwentów Państwa szkoły nie była wystarczająca (poniżej 10 osób), żeby możliwe było przedstawienie wyników zgodnie z zasadą chroniącą anonimowość absolwentów.")
}
```
...
# `tab_s3_zaw` - muszę odfiltrować zawody o liczebności n>=10 i jak to zrobię to usuwam komentarz
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  if (szk$tab_s3_zaw[[1]]$n > 0) {
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  if (nrow(tab_s3_zaw_check) > 0) {
    ...
    for (i in tab_s3_zaw_check$typ) {
      ...
      if (nrow(tab_god) > 0) {
        zawod = wrapper(tab_god$nazwa_zaw, 35)
        ...
        tab_god$typ = paste0("Pozostałe branżowye szkoły I stopnia\nz ", powiaty_dop)
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  sty_sier = c(
    "styczniu" = round(szk$S3_01[[1]]$ucz_prac * 100),
    "lutym" = round(szk$S3_02[[1]]$ucz_prac * 100), 
    "marcu" = round(szk$S3_03[[1]]$ucz_prac * 100),
    "kwietniu" = round(szk$S3_04[[1]]$ucz_prac * 100),
    "maju" = round(szk$S3_05[[1]]$ucz_prac * 100),
    "czerwcu" = round(szk$S3_06[[1]]$ucz_prac * 100),
  ...
  cat(
    ...
    if (length(unique(wrz_gru)) %in% 1) {
      ...
      paste0(
        ...
        " do ",
        max(wrz_gru),
        ifelse(unique(names(wrz_gru[wrz_gru == max(wrz_gru)])) %in% "wrześniu", paste0("% we "), paste0("% w ")),
...
```{r, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, results='asis', fig.align=wyrownanieTabWykr}
if (round(szk$E2_nauka_kontyn[[1]]$bs2 * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci) {
  if (szk$liczebnosc_branze_kont[[1]]$n > 0) {
    cat("**Tabela ",
        tab_num(),
...
if (length(szk$branze_zawody[[1]]) > 1 & l_col > 0) {
  ...
  szk$branze_zawody[[1]] %>% 
    ...
    add_header_above(header = c(" ", wiersze_nag))
} 
...
if (szk$Z4_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    "Absolwenci Państwa szkoły, którzy uczyli się w kolejnej szkole ponadpodstawowej, pracowali średnio w ciągu ",
    round(szk$Z4_ucz[[1]]$srednia * 100),
    "% okresu od września do grudnia ",
    params$rok,
    " roku. Dla kontynuujących naukę absolwentów pozostałych branżowych szkół I stopnia z ",
    powiaty_dop,
  ...
  cat("Liczba absolwentów Państwa szkoły uczących się w kolejnej szkole ponadpodstawowej nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.\n\n")
}
```
...
if (szk$Z4_ucz[[1]]$n >= progLiczebnosci) {  
  ...
  wykres = wyk_facet(tab_wyk,
                     wrapper(paste0("Wykres ",
                                    wyk_num(),
                                    ": Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów Państwa szkoły oraz absolwentów pozostałych branżowych szkół I stopnia z ",
                                    powiaty_dop,
...
if (szk$Z4_nie_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    "W okresie od września do grudnia ",
    params$rok,
    " roku absolwenci Państwa szkoły, którzy nie byli wtedy uczniami w kolejnej szkole ponadpodstawowej, pracowali średnio w ciągu ",
    round(szk$Z4_nie_ucz[[1]]$srednia * 100),
    "% tego okresu. Dla niekontynuujących nauki w szkole ponadpodstawowej absolwentów pozostałych branżowych szkół I stopnia z ",
    powiaty_dop,
    " średnia arytmetyczna odsetka przepracowanych miesięcy wyniosła ",
  ...
  if (length(przeprac_ods$name[przeprac_ods$value %in% max(przeprac_ods$value)]) %in% 1) {
    ...
    cat(
      "Wśród absolwentów Państwa szkoły niekontynuujących nauki wszystkie grupy osób, wyróżnione ze względu na przepracowaną część okresu od września do grudnia ",
      params$rok,
      " roku były równoliczne. Każda z tych grup stanowiła ",
      round(max(przeprac_ods$value) * 100),
      "% absolwentów Państwa szkoły.\n\n",
  ...
  cat("Liczba absolwentów Państwa szkoły nie uczących się w kolejnej szkole ponadpodstawowej nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.\n\n")
}
```
...
if (szk$Z8_formy_ucz[[1]]$n >= progLiczebnosci) {
  ...
  cat("Liczba absolwentów Państwa szkoły pracujących i uczących się w kolejnej szkole ponadpodstawowej nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.\n\n")
}
```
...
if (szk$Z8_formy_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Formy wykonywania pracy przez absolwentów Państwa szkoły oraz pozostałych branżowych szkół I stopnia z ",
                           powiaty_dop,
                           ", w grudniu ",
                           params$rok,
                           " roku - kontynuujący naukę"),
...
if (szk$Z8_formy_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  cat(
    "Spośród pracujących absolwentów Państwa szkoły, którzy nie uczyli się w kolejnej szkole ponadpodstawowej, w grudniu ",
    params$rok,
    " roku dla ",
    ...
    " absolwenci pozostałych branżowych szkół I stopnia z ",
    powiaty_dop,
  ...
  cat("Liczba absolwentów Państwa szkoły pracujących i niekontynuujących nauki w szkole ponadpodstawowej nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.\n\n")
}
```
...
if (szk$Z8_formy_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wykres = wykres_pion_skum(tab_wyk, wyk_tyt) +
    scale_fill_manual(values = c("#90cf60", # zielony
                                 "#d8ef8a", # jasnozielony
                                 "#f8dd00", # żółty
                                 "#fb8c59")) + # pomarańczowy
    ylab("Odsetek absolwentów")
...
## Kontynuowanie pracy we wrześniu `r params$rok` roku u pracodawcy zatrudniającego absolwenta jako młodocianego

```{r, echo=FALSE, message=FALSE, results='asis', fig.align=wyrownanieTabWykr, warning=FALSE}
if (szk$Z9_mlod_ucz[[1]]$n + szk$Z9_mlod_nie_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    "Wśród uczniów Państwa szkoły, którzy ukończyli ją w roku ",
    rocznik,
    " ",
    round(((szk$Z9_mlod_ucz[[1]]$n + szk$Z9_mlod_nie_ucz[[1]]$n) / szk$l_abs[[1]]) * 100),
    "% uczyło się w niej odbywając praktyczną naukę zawodu na podstawie umowy o pracę w celu przygotowania zawodowego, zawartej z pracodawcą.",
    sep = ""
  )
} else {
...
```{r, echo=FALSE, message=FALSE, results='asis', fig.align=wyrownanieTabWykr, warning=FALSE}
if (szk$Z9_mlod_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    "We wrześniu ",
    params$rok,
    " roku spośród absolwentów, którzy w Państwa szkole uczyli się jako młodociani pracownicy, a następnie kontynuowali naukę w kolejnej szkole ponadpodstawowej, ",
    round(szk$Z9_mlod_ucz[[1]]$kontuop * 100),
    "% było zatrudnionych u tego samego pracodawcy, na podstawie umowy o pracę.\n\n",
    "Ponadto wśród niekontynuujących nauki absolwentów Państwa szkoły, którzy uczyli się w niej jako młodociani pracownicy, ",
    round(szk$Z9_mlod_ucz[[1]]$niekontuop * 100),
    "% pracowało dla tego samego pracodawcy na podstawie innej niż umowa o pracę.",
    sep = ""
  )
  
  etykiety = c(
    "Uczący się, niekontynuujący pracy u danego\npracodawcy na podstawie umowy o pracę",
    "Uczący się, kontynuujący pracę u danego\npracodawcy na podstawie umowy o pracę"
  )
  
  if (length(god$Z9_mlod_ucz[[1]]) > 0) {
    tab_wyk = tab_wykres(szk$Z9_mlod_ucz[[1]],
                         god$Z9_mlod_ucz[[1]],
                         NULL,
                         powiaty_dop,
                         etykiety,
    ...
    wyk_tyt = wrapper(paste0("Wykres ",
                             wyk_num(),
                             ": Kontynuowanie pracy u pracodawców, u których absolwenci Państwa szkoły oraz pozostałych branżowych szkół I stopnia z ",
                             powiaty_dop,
                             " odbywali praktyczną naukę zawodu jako młodociani pracownicy, we wrześniu ",
                             params$rok,
                             " roku - młodociani kontynuujący naukę (%)"),
                      width = 85)
  } else {
    tab_wyk = tab_wykres(szk$Z9_mlod_ucz[[1]],
                         NULL,
    ...
    wyk_tyt = wrapper(paste0("Wykres ",
                             wyk_num(),
                             ": Kontynuowanie pracy u pracodawców, u których absolwenci Państwa szkoły odbywali praktyczną naukę zawodu jako młodociani pracownicy, we wrześniu ",
                             params$rok,
                             " roku - młodociani kontynuujący naukę (%)"),
                      width = 85)
  }
  
  tab_wyk$name = factor(tab_wyk$name, levels = c("Uczący się, niekontynuujący pracy u danego\npracodawcy na podstawie umowy o pracę", "Uczący się, kontynuujący pracę u danego\npracodawcy na podstawie umowy o pracę"))
  wykres = wykres_poziomy_statusy(tab_wyk, wyk_tyt) +
    scale_fill_manual(values = c("#d8ef8a", # jasnozielony 
                                 "#fb8c59")) # pomarańczowy
  
  levels(tab_wyk$typ) = linebreak(levels(tab_wyk$typ), align = "c")
  ...
  if (!god$Z9_mlod_ucz[[1]] > 0) {
    cat("Wyniki dla grupy odniesienia nie zostały przedstawione ze względu na zbyt małą liczbę absolwentów (mniej niż 10) lub szkół (mniej niż 3)")
  }
} else {
  cat(
    "Liczba absolwentów Państwa szkoły, którzy uczyli się w niej jako młodociani pracownicy, a we wrześniu ",
    params$rok,
    " roku uczyli się w kolejnej szkole ponadpodstawowej, nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość respondentów.",
    sep = ""
  )
}
```
...
```{r, echo=FALSE, message=FALSE, results='asis', fig.align=wyrownanieTabWykr, warning=FALSE}
if (szk$Z9_mlod_nie_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    "We wrześniu ",
    params$rok,
    "roku spośród absolwentów, którzy w Państwa szkole uczyli się jako młodociani pracownicy, a następnie nie kontynuowali nauki w kolejnej szkole ponadpodstawowej, ",
    round(szk$Z9_mlod_nie_ucz[[1]]$kont_uop * 100),
    "% było zatrudnionych na podstawie umowy o pracę u tego samego pracodawcy, z którym w okresie nauki w Państwa szkole mieli zawartą umowę o pracę w celu przygotowania zawodowego.\n\n",
    "Ponadto wśród niekontynuujących nauki absolwentów Państwa szkoły, którzy uczyli się w niej jako młodociani pracownicy, ",
    round(szk$Z9_mlod_nie_ucz[[1]]$kont_inne * 100),
    "% pracowało dla tego samego pracodawcy na podstawie innej niż umowa o pracę.",
    sep = ""
  )
  
  etykiety = c(
    "Nieuczący się, niekontynuujący pracy u danego pracodawcy",
    "Nieuczący się, kontynuujący pracę u danego pracodawcy na\npodstawie umowy o pracę",
    "Nieuczący się, kontynuujący pracę u danego pracodawcy w\nformie innej niż umowa o pracę"
  )
  
  if (length(god$Z9_mlod_ucz[[1]]) > 0) {
    tab_wyk = tab_wykres(szk$Z9_mlod_nie_ucz[[1]],
                         god$Z9_mlod_nie_ucz[[1]],
                         NULL,
                         powiaty_dop,
                         etykiety,
                         szk$typ_szk[[1]],
    ...
    wyk_tyt = wrapper(paste0("Wykres ",
                             ...
                             " odbywali praktyczną naukę zawodu jako młodociani pracownicy, we wrześniu ",
                             params$rok,
                             " roku - młodociani niekontynuujący nauki (%)"),
                      width = 85)
  } else {
    tab_wyk = tab_wykres(szk$Z9_mlod_nie_ucz[[1]],
                         NULL,
  ...
  tab_wyk$name = factor(tab_wyk$name, levels = c(
    "Nieuczący się, niekontynuujący pracy u danego pracodawcy",
    "Nieuczący się, kontynuujący pracę u danego pracodawcy w\nformie innej niż umowa o pracę",
    "Nieuczący się, kontynuujący pracę u danego pracodawcy na\npodstawie umowy o pracę"
  ))
  ...
  cat(
    "Liczba absolwentów Państwa szkoły, którzy uczyli się w niej jako młodociani pracownicy, a we wrześniu ",
    params$rok,
    " roku nie uczyli się w kolejnej szkole ponadpodstawowej, nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość respondentów.",
    sep = ""
  )
}
```
...
if (szk$W3_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% średnich wynagrodzeń mieszkańców powiatów, w których mieszkali.\n\nDla pracujących na etat i kontynuujących edukację absolwentów pozostałych baranżowych szkół I stopnia z ",
    powiaty_dop,
  ...
  cat("Liczba absolwentów Państwa szkoły pracujących na etat i kontynuujących naukę w szkole ponadpodstawowej nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.")
}
```
...
if (szk$W3_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Średni miesięczny przychód brutto z pracy uzyskiwany przez absolwentów Państwa szkoły oraz pozostałych branżowych szkół I stopnia w ",
                           powiaty_dop,
...
if (szk$W3_nie_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% średnich wynagrodzeń mieszkańców powiatów, w których mieszkali.\n\nDla pracujących na etat i niekontynuujących edukacji absolwentów pozostałych branżowych szkół I stopnia z ",
    powiaty_dop,
  ...
  cat("Liczba absolwentów Państwa szkoły pracujących na etat i niekontynuujących nauki w szkole ponadpodstawowej nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.")
}
```
...
if (szk$W3_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Średni miesięczny przychód brutto z pracy uzyskiwany przez absolwentów Państwa szkoły oraz pozostałych branżowych szkół I stopnia z ",
                           powiaty_dop,
                           " w odniesieniu do przeciętnego miesięcznego wynagrodzenia brutto w poszczególnych zamieszkiwanych przez nich powiatach, od września do grudnia ",
                           params$rok,
                           " roku - pracujący i niekontynuujący nauki"),
...
if (szk$l_abs[[1]] >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Odsetek absolwentów Państwa szkoły oraz pozostałych branżowych szkół I stopnia z ",
                           powiaty_dop,
...
Wskaźnik mierzy długość okresu, w ciągu którego absolwenci Państwa szkoły nie kontynuowali nauki a zarazem byli ubezpieczeni w ZUS z innego tytułu, niż wykonywanie pracy oraz niż status osoby bezrobotnej zarejestrowanej w urzędzie pracy. Przy obliczaniu wskaźnika nie uwzględniono osób, które nie kontynuowały nauki, nie pracowały ani nie były ubezpieczone w ZUS.
...
# 6. Aneks: wyniki dla absolwentów branżowych szkół I stopnia z całego kraju
...
W aneksie zaprezentowano tabelaryczne rozkłady wybranych wskaźników przedstawionych w raporcie, ale dla ogółu absolwentów, którzy w roku szkolnym `r rocznik` ukończyli branżowe szkoły I stopnia (BS I) w całym kraju.
...
god2$S3_12[[1]] %>%
  ...
  mutate(across(n, ~linebreak(paste0("Absolwenci BS I\nw całym  kraju"), align = "l"))) %>%
  mutate(across(tylko_ucz:neet,
                ~paste0(round(. * 100), "\\%"))) %>%
  kable(format = "latex", booktabs = TRUE, escape = FALSE, longtable = TRUE,
        linesep = "",
...
if (nrow(tabela) > 0) {
  ...
  cat("W żadnym zawodzie, w którym absolwenci ukończyli kształcenie w Państwa szkole, na poziomie wszystkich branżowych szkół I stopnia nie udało się zebrać minimum 10 absolwentów, aby móc przedstawić wyniki.")
}
```
...
tabelka %>% 
  ...
  kable_styling(position = wyrownanieTabWykr,
                ...
                full_width = FALSE,
                repeat_header_text = "(ciąg dalszy tabeli)") %>% 
  add_header_above(c("Sposób kontynuowania kształcenia", "Udział kontynuujących kształcenie wśród\nabsolwentów BS I z całego kraju" = 1),
                   align = c("l", "c"))
```

Odsetki mogą sumować się do więcej niż 100% ze względu na możliwość kontynuowania edukacji w więcej niż jednej formie.

3. Praca po ukończeniu branżowej szkoły I stopnia w okresie od września do grudnia `r params$rok` roku

Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów branżowych szkół I stopnia z całego kraju.
...
cbind(ucz, n_ucz[,2]) %>% 
  ...
  kable_styling(position = wyrownanieTabWykr,
                ...
                full_width = FALSE,
                repeat_header_text = "(ciąg dalszy tabeli)") %>%
  add_header_above(c(" ", "Absolwenci BS I z całego\nkraju kontynuujący naukę" = 1, "Absolwenci BS I z całego\nkraju niekontynuujący nauki" = 1),
                   align = c("l", "c", "c"))
...
Procentowy udział grup wyróżnionych ze względu na formę wykonywania pracy wśród pracujących absolwentów branżowych szkół I stopnia z całego kraju.
...
Procentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli zarejestrowani jako bezrobotni, wśród absolwentów branżowych szkół I stopnia z całego kraju.
...
Procentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli bierni edukacyjnie i zawodowo, wśród absolwentów branżowych szkół I stopnia z całego kraju.
