...
## Dynamiczne grupy odniesienia

Tutaj opisać ideę tych grup i jakie zasady rządzą doborem wyświetlanej grupy odniesienia
...
Wskaźnik prezentowany w tym rozdziale wykorzystuje 4 statusy absolwentów opisujące ich sytuację edukacyjno-zawodową w różnych momentach. Poniższa tabela przybliża konstrukcję poszczególnych statusów.
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  ...
  wykres_poziomy_statusy(
    ...
    wrapper(paste0("Wykres ",
                   wyk_num(),
                   ": Status edukacyjno-zawodowy absolwentów Państwa szkoły oraz pozostałych branżowych szkół w grupach odniesienia, w grudniu ",
                   params$rok,
                   " roku - ogółem"), 80))
}
```
...
if (szk$Z4_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           ...
                           odmiany$god_dop,
                           " i w kraju, od września do grudnia ", params$rok, " - uczący się w szkołach ponadpodstawowych."),
                    width = 85)
...
tab_wyk = tab_wykres(szk$Z4_nie_ucz[[1]],
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                         wyk_num(),
                         ": Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów Państwa szkoły oraz pozostałych branżowych szkół I stopnia w województwie i w kraju, od września do grudnia ", params$rok, " - nie uczący się w szkołach ponadpodstawowych."),
                  width = 85)
...
if (szk$W1_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    "W przypadku absolwentów Państwa szkoły, którzy kontynuowali naukę w szkole ponadpodstawowej i przez co najmniej część okresu od września do grudnia ",
    params$rok,
    ...
    " zł. To znaczy, że co najmniej połowa grupy, dla której dostępne były informacje o zarobkach, uzyskiwała średni miesięczny przychód nieprzekraczający tej kwoty, a co najmniej połowa - przychód w wysokosci co najmniej tej kwoty. W przypadku absolwentów pozostałych branżowych szkół I stopnia z ",
    odmiany$god_dop,
...
if (szk$W1_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Miesięczne przychody brutto z pracy absolwentów Państwa szkoły oraz pozostałych branżowych szkół I stopnia w ",
                           odmiany$god_dop,
...
if (szk$W1_nie_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    " zł. To znaczy, że co najmniej połowa grupy, dla której dostępne były informacje o zarobkach, uzyskiwała średni miesięczny przychód nieprzekraczający tej kwoty, a co najmniej połowa - przychód w wysokości co najmniej tej kwoty. W przypadku absolwentów pozostałych branżowych szkół I stopnia z ",
    odmiany$god_dop,
...
if (szk$W1_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Miesięczne przychody brutto absolwentów Państwa szkoły oraz pozostałych branżowych szkół w ",
                           odmiany$god_dop,
                           " i w kraju, od września do grudnia ",
                           params$rok,
  ...
  wykres = boxplot_zl(tab_wyk, wyk_tyt) +
    ylab(wrapper(paste0("Średni miesięczny przychód z pracy w okresie\nod grudnia ",
         params$rok,
         " roku (zł)"), 45)) +
    geom_text(data = wyk_lab,
              aes(x = typ, y = value, label = lab),
