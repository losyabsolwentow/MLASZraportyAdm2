...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  ...
  cat("Liczba absolwentów Państwa szkoły nie była wystarczająca (mniej niż 10), żeby możliwe było przedstawienie wyników zgodnie z zasadą chroniącą anonimowość absolwentów.")
}
```
...
if (szk$Z4_ucz[[1]]$n >= progLiczebnosci) {  
  ...
  wykres = wyk_facet(tab_wyk,
                     wrapper(paste0("Wykres ",
                                    wyk_num(),
                                    ": Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów Państwa szkoły oraz absolwentów pozostałych branżowych szkół I stopnia w ",
                                    powiaty_dop,
...
if (szk$Z8_formy_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Formy wykonywania pracy przez absolwentów Państwa szkoły oraz pozostałych branżowych szkół I stopnia w ",
                           powiaty_dop,
                           ", w grudniu ",
                           params$rok,
                           " roku - kontynuujący naukę"),
...
if (szk$Z9_mlod_ucz[[1]]$n + szk$Z9_mlod_nie_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    rocznik,
    " ",
    round((szk$Z9_mlod_ucz[[1]]$n + szk$Z9_mlod_nie_ucz[[1]]$n) / szk$l_abs[[1]]),
...
if (szk$Z9_mlod_ucz[[1]]$n >= progLiczebnosci) {
  ...
  if (god$Z9_mlod_ucz[[1]] > 0) {
    ...
    wyk_tyt = wrapper(paste0("Wykres ",
                             wyk_num(),
                             ": Kontynuowanie pracy u pracodawców, u których absolwenci Państwa szkoły oraz pozostałych branżowych szkół I stopnia w ",
                             powiaty_dop,
  ...
  wykres = wykres_poziomy_statusy(tab_wyk, wyk_tyt) +
    scale_fill_manual(values = c("#d62f26", "#fb8c59"))
...
if (szk$Z9_mlod_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wykres = wykres_poziomy_statusy(tab_wyk, wyk_tyt) +
    scale_fill_manual(values = c("#f8dd00", "#d8ef89", "#90cf60"))
...
if (szk$l_abs[[1]] >= progLiczebnosci) {
  ...
  cat("Liczba absolwentów Państwa szkoły nie była wystarczająca (poniżej 10 osób), żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.")
}
```
...
Wskaźnik mierzy długość okresu, w ciągu którego absolwenci Państwa szkoły nie pracowali, nie uczyli się w kolejnej szkole ponadpodstawowej ani nie byli zarejestrowani jako osoby bezrobotne.
...
Procentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli zarejestrowani jako bezrobotni, wśród absolwentów barnżowych szkół I stopnia z całego kraju.
...
Procentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli bierni edukacyjnie i zawodowo, wśród absolwentów barnżowych szkół I stopnia z całego kraju.
