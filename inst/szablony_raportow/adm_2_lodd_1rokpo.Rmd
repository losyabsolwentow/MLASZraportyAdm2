---
title: |
  | Raport roczny z monitoringu karier absolwentów, którzy ukończyli liceum ogólnokształcące dla dorosłych w roku szkolnym `r params$rok - 1`/`r params$rok`
subtitle: |
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "%, podczas gdy spośród absolwentów pozostałych liceów dla dorosłych z ",
    powiaty_dop,
    " ",
    ...
    " wśród absolwentów pozostałych liceów dla dorosłych z ", 
    powiaty_dop,
    " (",
    ...
    "% absolwentów pozostałych liceów dla dorosłych z ",
    powiaty_dop,
    ".\n",
    sep = ""
  )
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  ...
  wykres = wykres_poziomy_statusy(
    ...
    wrapper(paste0("Wykres ",
                   wyk_num(),
                   ": Status edukacyjno-zawodowy absolwentów Państwa szkoły oraz absolwentów pozostałych liceów ogólnokształcących dla dorosłych z ",
                   powiaty_dop,
                   ", w grudniu ",
                   params$rok,
                   " roku - ogółem"), 80))
...
# 4. Praca po ukończeniu liceum dla dorosłych
...
if (length(szk$Z4_ucz[[1]]) > 0) {
  if (szk$Z4_ucz[[1]]$n >= progLiczebnosci) {
    cat(
      ...
      params$rok,
      " roku. Dla kontynuujących naukę absolwentów pozostałych liceów ogólnokształcących dla dorosłych z ",
      powiaty_dop,
...
if (length(szk$Z4_ucz[[1]]) > 0) {
  if (szk$Z4_ucz[[1]]$n >= progLiczebnosci) {
    ...
    wykres = wyk_facet(tab_wyk,
                       wrapper(paste0("Wykres ",
                                      wyk_num(),
                                      ": Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów Państwa szkoły oraz absolwentów pozostałych liceów ogólnokształcących dla dorosłych w ",
                                      powiaty_dop,
...
if (length(szk$Z4_nie_ucz[[1]]) > 0) {
  if (szk$Z4_nie_ucz[[1]]$n >= progLiczebnosci) {
    cat(
      ...
      "% tego okresu. Dla niekontynuujących nauki na uczelni ani w szkole policealnej absolwentów pozostałych liceów ogólnokształcących dla dorosłych z ",
      powiaty_dop,
      " średnia arytmetyczna odsetka przepracowanych miesięcy wyniosła ",
...
if (szk$Z8_formy_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% łączyło różne formy.\n\nWśród kontynuujących naukę absolwentów pozostałych liceów ogólnokształcących dla dorosłych z ",
    powiaty_dop,
...
if (szk$Z8_formy_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Formy wykonywania pracy przez absolwentów Państwa szkoły oraz pozostałych liceów ogólnokształcących dla dorosłych w ",
                           powiaty_dop,
                           ", w grudniu ",
                           params$rok,
                           " roku - kontynuujący naukę"),
...
if (szk$Z8_formy_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  cat(
    ...
    " absolwenci pozostałych liceów ogólnokształcących dla dorosłych z ",
    powiaty_dop,
...
if (szk$W3_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% średnich wynagrodzeń mieszkańców powiatów, w których mieszkali.\n\nDla pracujących na etat i kontynuujących edukację absolwentów pozostałych liceów ogólnokształcących dla dorosłych z ",
    powiaty_dop,
...
if (szk$W3_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Średni miesięczny przychód brutto z pracy uzyskiwany przez absolwentów Państwa szkoły oraz pozostałych liceów ogólnokształcących dla dorosłych z ",
                           powiaty_dop,
...
if (szk$W3_nie_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% średnich wynagrodzeń mieszkańców powiatów, w których mieszkali.\n\nDla pracujących na etat i niekontynuujących edukacji absolwentów pozostałych liceów ogólnokształcących dla dorosłych z ",
    powiaty_dop,
...
if (szk$W3_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Średni miesięczny przychód brutto z pracy uzyskiwany przez absolwentów Państwa szkoły oraz pozostałych liceów ogólnokształcących dla dorosłych w ",
                           powiaty_dop,
                           " w odniesieniu do przeciętnego miesięcznego wynagrodzenia brutto w poszczególnych zamieszkiwanych przez nich powiatach, od września do grudnia ",
                           params$rok,
...
if (szk$l_abs[[1]] >= progLiczebnosci) {
  cat(
    ...
    "% pozostawało w rejestrze osób bezrobotnych przez cały ten okres.\n\nWśród absolwentów pozostałych liceów ogólnokształcących dla dorosłych z ",
    powiaty_dop,
...
if (szk$l_abs[[1]] >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Odsetek absolwentów Państwa szkoły oraz pozostałych liceów ogólnokształcących dla dorosłych z ",
                           powiaty_dop,
...
# 6. Aneks: wyniki dla absolwentów liceów ogólnokształcących dla dorosłych z całego kraju
...
W aneksie zaprezentowano tabelaryczne rozkłady wybranych wskaźników przedstawionych w raporcie, ale dla ogółu absolwentów, którzy w roku szkolnym `r rocznik` ukończyli licea ogólnokształcące dla dorosłych w całym kraju.
...
god2$S3_12[[1]] %>%
  ...
  mutate(across(n, ~paste0("Absolwenci liceów ogólnokształcących\ndla dorosłych w całym kraju"))) %>%
  mutate(across(tylko_ucz:neet,
                ~paste0(round(. * 100), "\\%"))) %>%
  kable(format = "latex", booktabs = TRUE, escape = FALSE, longtable = TRUE,
        linesep = "",
...
tabelka %>% 
  ...
  kable_styling(position = wyrownanieTabWykr,
                ...
                full_width = FALSE,
                repeat_header_text = "(ciąg dalszy tabeli)") %>% 
  add_header_above(c("Sposób kontynuowania kształcenia", "Udział kontynuujących kształcenie wśród\nabsolwentów liceów dla dorosłych z całego kraju" = 1),
                   align = c("l", "c"))
```

Odsetki mogą sumować się do więcej niż 100% ze względu na możliwość kontynuowania edukacji w więcej niż jednej formie.

3. Praca po ukończeniu liceum ogólnokształcącego dla dorosłych w okresie od września do grudnia `r params$rok` roku

Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów liceów ogólnokształcących dla dorosłych z całego kraju.
...
cbind(ucz, n_ucz[,2]) %>% 
  ...
  kable_styling(position = wyrownanieTabWykr,
                ...
                full_width = FALSE,
                repeat_header_text = "(ciąg dalszy tabeli)") %>%
  add_header_above(c(" ", "Absolwenci liceów dla dorosłych z całego\nkraju kontynuujący naukę" = 1, "Absolwenci liceów dla dorosłych z całego\nkraju niekontynuujący nauki" = 1),
                   align = c("l", "c", "c"))
...
cbind(ucz, n_ucz[,2]) %>% 
  ...
  kable("latex", booktabs = TRUE, escape = FALSE, longtable = TRUE,
    ...
    kable_styling(position = wyrownanieTabWykr,
                  ...
                  full_width = FALSE,
                  repeat_header_text = "(ciąg dalszy tabeli)") %>%
    add_header_above(c(" ", "Absolwenci liceów dla dorosłych z całego\nkraju kontynuujących naukę" = 1, "Absolwenci liceów dla dorosłych z całego\nkraju niekontynuujący nauki" = 1),
                   align = c("l", "c", "c"))
```
...
Procentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli zarejestrowani jako bezrobotni, wśród absolwentów liceów ogólnokształcących dla dorosłych z całego kraju.
...
Procentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli bierni edukacyjnie i zawodowo, wśród absolwentów liceów ogólnokształcących dla dorosłych z całego kraju.
