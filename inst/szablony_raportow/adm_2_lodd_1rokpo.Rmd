...
god2$S3_12[[1]] %>%
  ...
  mutate(across(n, ~linebreak(paste0("Absolwenci liceów ogólnokształcących\ndla dorosłych w całym kraju"), align = "l"))) %>%
  mutate(across(tylko_ucz:neet,
                ~paste0(round(. * 100), "\\%"))) %>%
  kable(format = "latex", booktabs = TRUE, escape = FALSE, longtable = TRUE,
        linesep = "",
...
cbind(ucz, n_ucz[,2]) %>% 
  ...
  kable("latex", booktabs = TRUE, escape = FALSE, longtable = TRUE,
    ...
    kable_styling(position = wyrownanieTabWykr,
                  ...
                  full_width = FALSE,
                  repeat_header_text = "(ciąg dalszy tabeli)") %>%
    add_header_above(c(" ", "Absolwenci liceów dla dorosłych z\ncałego kraju kontynuujący naukę" = 1, "Absolwenci liceów dla dorosłych\nz całego kraju niekontynuujący nauki" = 1),
                   align = c("l", "c", "c"))
```
