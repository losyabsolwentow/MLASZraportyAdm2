---
title: |
  | Raport roczny z monitoringu karier absolwentów, którzy ukończyli szkołę specjalną przysposabiającą do pracy w roku szkolnym `r params$rok - 1`/`r params$rok`
subtitle: |
...
Raporty szkolne pierwotnie były zaprojektowane dla szkół kształcących zawodowo. W przypadku innych typów szkół, które nie prowadzą kształcenia w zawodach, niektóre zestawienia nie są możliwe do wygenerowania.

## Główne zmiany w stosunku do pierwszej edycji raportów

Pierwsza edycja monitoringu karier dotyczyła wyłącznie absolwentów szkół kształcących zawodowo. W niniejszej edycji po raz pierwszy przygotowano raporty również dla liceów ogólnokształcących dla młodzieży, liceów ogólnokształcących dla dorosłych oraz szkół specjalnych przysposabiających do pracy. Żeby zwiększyć przydatność raportów, po rozważaniu uwag zgłoszonych przez dyrektorów szkół, nauczycieli i doradców zawodowych, wprowadziliśmy zmiany w stosunku do pierwszej edycji. Polegają one przede wszystkim na:
...
* uzupełnieniu raportów o zestawienie sytuacji edukacyjno-zawodowej absolwentów szkoły wykształconych w poszczególnych zawodach z sytuacją absolwentów z grupy odniesienia wykształconych w tych samych zawodach^[Dotyczy BS I, techników i szkół policealnych], 

* uzupełnieniu raportów o informacje na temat dyscyplin, w których podjęli studia absolwenci wykształceni w poszczególnych zawodach (o ile liczebność grup wyróżnionych ze względu na wyuczony zawód na to pozwala)^[Dotyczy techników i szkół policealnych],

* uproszczeniu sposobu prezentacji sytuacji absolwentów poprzez zmniejszenie liczby statusów odzwierciedlających ich sytuację edukacyjną i zawodową z sześciu do czterech; kategoria “uczy się i brak informacji o sytuacji zawodowej” została połączona z kategorią “uczy się i nie pracuje na podstawie umowy o pracę ani samozatrudnienia”, a kategoria “nie uczy się i brak informacji o sytuacji zawodowej” z kategorią  “nie uczy się i nie pracuje”,
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "%, podczas gdy spośród absolwentów pozostałych szkół specjalnych przysposabiających do pracy z ",
    powiaty_dop,
    " ",
    ...
    " wśród absolwentów pozostałych szkół specjalnych przysposabiających do pracy z ", 
    powiaty_dop,
    " (",
    round(god$S3_12[[1]]$ucz_prac * 100),
    "%), z tego samego rocznika.\n\nW tym samym miesiącu ",
    round(szk$S3_12[[1]]$tylko_prac * 100),
    "% absolwentów Państwa liceum pracowało^[Na podstawie umowy o pracę, umowy cywilnoprawnej lub prowadząc własną działalność gospodarczą.], ale nie studiowało ani nie uczęszczało do szkoły ponadpodstawowej. Do grupy pracujących, ale nie kontynuujących edukacji formalnej należało ",
    ...
    "% nie kontynuowało nauki w szkole ponadpodstawowej lub na studiach ani nie wykonywało pracy odnotowywanej przez ZUS^[Na podstawie umowy o pracę, na podstawie umowy cywilnoprawnej ani prowadząc własną działalność gospodarczą.]. W takiej samej sytuacji było ",
    round(god$S3_12[[1]]$neet * 100),
    "% absolwentów pozostałych szkół specjalnych przysposabiających do pracy z ",
    powiaty_dop,
    ".\n",
    sep = ""
  )
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  ...
  wykres = wykres_poziomy_statusy(
    ...
    wrapper(paste0("Wykres ",
                   wyk_num(),
                   ": Status edukacyjno-zawodowy absolwentów Państwa szkoły oraz absolwentów pozostałych szkół specjalnych przysposabiających do pracy z ",
                   powiaty_dop,
                   ", w grudniu ",
                   params$rok,
                   " roku - ogółem"), 80))
...
if (szk$E2_nauka_kontyn[[1]]$n >= progLiczebnosci) {
  ...
  if (sposoby != 0) {
    ...
    cat("Żaden z absolwentów Państwa szkoły z rocznika ",
        ...
        " w grudniu ",
        params$rok,
        " roku nie kontynuował nauki studiując, ucząc się w szkole policealnej, na Kwalifikacyjnym Kursie Zawodowym ani na Kursie Umiejętności Zawodowych.",
        sep = "")
  }
} else {
...
if (szk$E2_nauka_kontyn[[1]]$n >= progLiczebnosci) {
  ...
  tab = tab_wykres(szk$E2_nauka_kontyn[[1]],
                   ...
                   80) %>%
    select(-typ) %>% 
    filter(!(name %in% c("bs1", "spolic", "studia"))) %>% 
    arrange(desc(value))
...
if (length(szk$Z4_ucz[[1]]) > 0) {
  if (szk$Z4_ucz[[1]]$n >= progLiczebnosci) {
    cat(
      "Absolwenci Państwa szkoły, którzy uczyli się w liceum dla dorosłych, pracowali średnio w ciągu ",
      round(szk$Z4_ucz[[1]]$srednia * 100),
      "% okresu od września do grudnia ",
      params$rok,
      " roku. Dla kontynuujących naukę absolwentów pozostałych szkół specjalnych przysposabiających do pracy z ",
      powiaty_dop,
...
if (length(szk$Z4_ucz[[1]]) > 0) {
  if (szk$Z4_ucz[[1]]$n >= progLiczebnosci) {
    ...
    wykres = wyk_facet(tab_wyk,
                       wrapper(paste0("Wykres ",
                                      wyk_num(),
                                      ": Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów Państwa szkoły oraz absolwentów pozostałych szkół specjalnych przysposabiających do pracy w ",
                                      powiaty_dop,
...
if (length(szk$Z4_nie_ucz[[1]]) > 0) {
  if (szk$Z4_nie_ucz[[1]]$n >= progLiczebnosci) {
    cat(
      ...
      "% tego okresu. Dla niekontynuujących nauki na uczelni ani w szkole policealnej absolwentów pozostałych szkół specjalnych przysposabiających do pracy z ",
      powiaty_dop,
      " średnia arytmetyczna odsetka przepracowanych miesięcy wyniosła ",
...
if (szk$Z8_formy_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    "W grudniu ",
    params$rok,
    " roku spośród absolwentów Państwa szkoły, którzy w liceum dla dorosłych, a zarazem pracowali, ",
    ...
    "% łączyło różne formy.\n\nWśród kontynuujących naukę absolwentów pozostałych szkół specjalnych przysposabiających do pracy z ",
    powiaty_dop,
  ...
  cat("Liczba absolwentów Państwa szkoły pracujących i kontynuujących naukę w liceum dla dorosłych nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.\n\n")
}
```
...
if (szk$Z8_formy_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Formy wykonywania pracy przez absolwentów Państwa szkoły oraz pozostałych szkół specjalnych przysposabiających do pracy w ",
                           powiaty_dop,
                           ", w grudniu ",
                           params$rok,
                           " roku - kontynuujący naukę"),
...
if (szk$Z8_formy_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  cat(
    "Spośród pracujących absolwentów Państwa szkoły, którzy nie kontynuowali nauki w liceum dla dorosłych, w grudniu ",
    params$rok,
    " roku dla ",
    ...
    " absolwenci pozostałych szkół specjalnych przysposabiających do pracy z ",
    powiaty_dop,
  ...
  cat("Liczba absolwentów Państwa szkoły pracujących i niekontynuujących nauki w liceum dla dorosłych nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.\n\n")
}
```
...
if (szk$W3_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% średnich wynagrodzeń mieszkańców powiatów, w których mieszkali.\n\nDla pracujących na etat i kontynuujących edukację absolwentów pozostałych szkół specjalnych przysposabiających do pracy z ",
    powiaty_dop,
  ...
  cat("Liczba absolwentów Państwa szkoły pracujących na etat i kontynuujących naukę nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.")
}
```
...
if (szk$W3_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Średni miesięczny przychód brutto z pracy uzyskiwany przez absolwentów Państwa szkoły oraz pozostałych szkół specjalnych przysposabiających do pracy z ",
                           powiaty_dop,
...
if (szk$W3_nie_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% średnich wynagrodzeń mieszkańców powiatów, w których mieszkali.\n\nDla pracujących na etat i niekontynuujących edukacji absolwentów pozostałych szkół specjalnych przysposabiających do pracy z ",
    powiaty_dop,
  ...
  cat("Liczba absolwentów Państwa szkoły pracujących na etat i niekontynuujących nauki w liceum dla dorosłych nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.")
}
```
...
if (szk$W3_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Średni miesięczny przychód brutto z pracy uzyskiwany przez absolwentów Państwa szkoły oraz pozostałych szkół specjalnych przysposabiających do pracy w ",
                           powiaty_dop,
                           " w odniesieniu do przeciętnego miesięcznego wynagrodzenia brutto w poszczególnych zamieszkiwanych przez nich powiatach, od września do grudnia ",
                           params$rok,
...
if (szk$l_abs[[1]] >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Odsetek absolwentów Państwa szkoły oraz pozostałych szkół specjalnych przysposabiających do pracy z ",
                           powiaty_dop,
...
Wskaźnik mierzy długość okresu, w ciągu którego absolwenci Państwa szkoły nie pracowali, nie studiowali, nie uczyli się w kolejnej szkole ani nie byli zarejestrowani jako osoby bezrobotne.
...
# 6. Aneks: wyniki dla absolwentów szkół specjalnych przysposabiających do pracy z całego kraju
...
W aneksie zaprezentowano tabelaryczne rozkłady wybranych wskaźników przedstawionych w raporcie, ale dla ogółu absolwentów, którzy w roku szkolnym `r rocznik` ukończyli licea ogólnokształcące dla młodzieży w całym kraju.

1. Status edukacyjno-zawodowy w grudniu `r params$rok` roku - ogółem

```{r, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, results='asis', fig.align=wyrownanieTabWykr}
god2$S3_12[[1]] %>%
  as_tibble() %>% 
  select(n, tylko_ucz, ucz_prac, tylko_prac, neet) %>% 
  mutate(across(n, ~paste0("Absolwenci szkół specjalnych\nw całym kraju"))) %>%
  mutate(across(tylko_ucz:neet,
                ~paste0(round(. * 100), "\\%"))) %>%
  kable(format = "latex", booktabs = TRUE, escape = FALSE, longtable = TRUE,
        linesep = "",
...
tabelka %>% 
  ...
  kable_styling(position = wyrownanieTabWykr,
                ...
                full_width = FALSE,
                repeat_header_text = "(ciąg dalszy tabeli)") %>% 
  add_header_above(c("Sposób kontynuowania kształcenia", "Udział kontynuujących kształcenie wśród\nabsolwentów szkół specjalnych z całego kraju" = 1),
                   align = c("l", "c"))
```

Odsetki mogą sumować się do więcej niż 100% ze względu na możliwość kontynuowania edukacji w więcej niż jednej formie.

3. Praca po ukończeniu liceum ogólnokształcącego dla młodzieży w okresie od września do grudnia `r params$rok` roku

Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów szkół specjalnych przysposabiających do pracy z całego kraju.
...
cbind(ucz, n_ucz[,2]) %>% 
  ...
  kable_styling(position = wyrownanieTabWykr,
                ...
                full_width = FALSE,
                repeat_header_text = "(ciąg dalszy tabeli)") %>%
  add_header_above(c(" ", "Absolwenci szkół specjalnych z całego\nkraju kontynuujący naukę" = 1, "Absolwenci szkół specjalnych z całego\nkraju niekontynuujący nauki" = 1),
                   align = c("l", "c", "c"))
...
Procentowy udział grup wyróżnionych ze względu na formę wykonywania pracy wśród pracujących absolwentów liceów ogólnokształcących z całego kraju.
...
cbind(ucz, n_ucz[,2]) %>% 
  ...
  kable("latex", booktabs = TRUE, escape = FALSE, longtable = TRUE,
    ...
    kable_styling(position = wyrownanieTabWykr,
                  ...
                  full_width = FALSE,
                  repeat_header_text = "(ciąg dalszy tabeli)") %>%
    add_header_above(c(" ", "Absolwenci szkół specjalnych z całego\nkraju kontynuujących naukę" = 1, "Absolwenci szkół specjalnych z całego\nkraju niekontynuujący nauki" = 1),
                   align = c("l", "c", "c"))
```
...
Procentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli zarejestrowani jako bezrobotni, wśród absolwentów szkół specjalnych przysposabiających do pracy z całego kraju.
...
Procentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli bierni edukacyjnie i zawodowo, wśród absolwentów szkół specjalnych przysposabiających do pracy z całego kraju.
