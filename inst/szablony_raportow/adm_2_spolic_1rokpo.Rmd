---
title: |
  | Raport roczny z monitoringu karier absolwentów, którzy ukończyli szkołę policealną w roku szkolnym `r params$rok - 1`/`r params$rok`
subtitle: |
...
* uzupełnieniu raportów o zestawienie sytuacji edukacyjno-zawodowej absolwentów wykształconych w poszczególnych zawodach z sytuacją absolwentów z grupy odniesienia wykształconych w tych samych zawodach, 

* uzupełnieniu raportów o informacje na temat dyscyplin, w których podjęli studia absolwenci wykształceni w poszczególnych zawodach (o ile liczebność grup wyróżnionych ze względu na wyuczony zawód na to pozwala),
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "%, podczas gdy spośród absolwentów pozostałych szkół policealnych z ",
    powiaty_dop,
    " ",
    ...
    "% absolwentów Państwa szkoły policealnej pracowało^[Na podstawie umowy o pracę, umowy cywilnoprawnej lub prowadząc własną działalność gospodarczą.], ale nie studiowało ani nie uczęszczało do kolejnej szkoły. Do grupy pracujących, ale nie kontynuujących edukacji formalnej należało ",
    ...
    "% nie kontynuowało nauki w kolejnej szkole policealnej, nie studiowało ani nie wykonywało pracy odnotowywanej przez ZUS^[Na podstawie umowy o pracę, na podstawie umowy cywilnoprawnej ani prowadząc własną działalność gospodarczą.]. W takiej samej sytuacji było ",
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  ...
  wykres = wykres_poziomy_statusy(
    ...
    wrapper(paste0("Wykres ",
                   wyk_num(),
                   ": Status edukacyjno-zawodowy absolwentów Państwa szkoły oraz absolwentów pozostałych szkół policealnych z ",
                   powiaty_dop,
                   ", w grudniu ",
                   params$rok,
                   " roku - ogółem"), 80))
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  if (nrow(tab_s3_zaw_check) > 0) {
    ...
    for (i in tab_s3_zaw_check$typ) {
      ...
      tab_god$typ = paste0("Pozostałe szkoły policealne\nz ", powiaty_dop)
...
if (szk$E2_nauka_kontyn[[1]]$n >= progLiczebnosci) {
  ...
  if (sposoby != 0) {
    ...
    cat("Żaden z absolwentów Państwa szkoły z rocznika ",
        ...
        " w grudniu ",
        params$rok,
        " roku nie kontynuował nauki studiując, ucząc się w kolejnej szkole policealnej, na Kwalifikacyjnym Kursie Zawodowym ani na Kursie Umiejętności Zawodowych.",
        sep = "")
  }
} else {
...
```{r, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, results='asis', fig.align=wyrownanieTabWykr}
if (length(szk$liczebnosc_dziedziny[[1]]) > 0) {
...
```{r, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, results='asis', fig.align=wyrownanieTabWykr}
if (length(szk$liczebnosc_dyscypliny[[1]]) > 0) {
  if (round(szk$E2_nauka_kontyn[[1]]$studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci) {
...
if (length(szk$dyscypliny_zawody[[1]]) > 1 & l_col > 0) {
  ...
  cat("\n\n**Tabela ",
      ...
      ":** Dyscypliny, w których absolwenci Państwa szkoły najczęściej studiowali w grudniu ",
      params$rok,
      " roku - w grupach wyróżnionych ze względu na zawód wyuczony w szkole policealnej (%)",
      sep = "")
...
if (length(szk$Z4_ucz[[1]]) > 0) {
  if (szk$Z4_ucz[[1]]$n >= progLiczebnosci) {
    cat(
      "Absolwenci Państwa szkoły, którzy studiowali lub uczyli się w kolejnej szkole policealnej, pracowali średnio w ciągu ",
      round(szk$Z4_ucz[[1]]$srednia * 100),
      "% okresu od września do grudnia ",
      params$rok,
      " roku. Dla kontynuujących naukę absolwentów pozostałych szkół policealnych z ",
      powiaty_dop,
    ...
    cat("Liczba absolwentów Państwa szkoły studiujących lub uczących się w kolejnej szkole policealnej nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.\n\n")
  }
} else {
  cat("Liczba absolwentów Państwa szkoły studiujących lub uczących się w kolejnej szkole policealnej nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.\n\n")
}
...
if (length(szk$Z4_ucz[[1]]) > 0) {
  if (szk$Z4_ucz[[1]]$n >= progLiczebnosci) { 
    ...
    wykres = wyk_facet(tab_wyk,
                       wrapper(paste0("Wykres ",
                                      wyk_num(),
                                      ": Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów Państwa szkoły oraz absolwentów pozostałych szkół policealnych w ",
                                      powiaty_dop,
...
if (length(szk$Z4_nie_ucz[[1]]) > 0) {
  if (szk$Z4_nie_ucz[[1]]$n >= progLiczebnosci) {
    cat(
      "W okresie od września do grudnia ",
      params$rok,
      " roku absolwenci Państwa szkoły, którzy nie byli wtedy studentami ani słuchaczami kolejnej szkoły policealnej, pracowali średnio w ciągu ",
    ...
    cat("Liczba absolwentów Państwa szkoły niestudiujących ani nieuczących się w kolejnej szkole policealnej nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.\n\n")
  }
} else {
  cat("Liczba absolwentów Państwa szkoły niestudiujących ani nieuczących się w kolejnej szkole policealnej nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.\n\n")
}
...
if (szk$Z8_formy_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Formy wykonywania pracy przez absolwentów Państwa szkoły oraz pozostałych szkół policealnych w ",
                           powiaty_dop,
                           ", w grudniu ",
                           params$rok,
                           " roku - kontynuujący naukę"),
...
if (szk$Z8_formy_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  cat(
    "Spośród pracujących absolwentów Państwa szkoły, którzy nie kontynuowali nauki na studiach ani w kolejnej szkole policealnej, w grudniu ",
    params$rok,
    " roku dla ",
    ...
    " absolwenci pozostałych szkół policealnych z ",
    powiaty_dop,
  ...
  cat("Liczba absolwentów Państwa szkoły pracujących i niekontynuujących nauki na uczelni ani w kolejnej szkole policealnej nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.\n\n")
}
```
...
if (szk$W3_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% średnich wynagrodzeń mieszkańców powiatów, w których mieszkali.\n\nDla pracujących na etat i kontynuujących edukację absolwentów pozostałych szkół policealnych z ",
    powiaty_dop,
  ...
  cat("Liczba absolwentów Państwa szkoły pracujących na etat i kontynuujących naukę na uczelni lub w kolejnej szkole policealnej nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.")
}
```
...
if (szk$W3_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Średni miesięczny przychód brutto z pracy uzyskiwany przez absolwentów Państwa szkoły oraz pozostałych szkół policealnych z ",
                           powiaty_dop,
...
if (szk$W3_nie_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% średnich wynagrodzeń mieszkańców powiatów, w których mieszkali.\n\nDla pracujących na etat i niekontynuujących edukacji absolwentów pozostałych szkół policealnych z ",
    powiaty_dop,
  ...
  cat("Liczba absolwentów Państwa szkoły pracujących na etat i niekontynuujących nauki na uczelni ani w kolejnej szkole policealnej nie była wystarczająca, żeby możliwe było przedstawienie wyników dla tej grupy, zgodnie z zasadą chroniącą anonimowość absolwentów.")
}
```
...
if (szk$W3_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Średni miesięczny przychód brutto z pracy uzyskiwany przez absolwentów Państwa szkoły oraz pozostałych szkół policealnych w ",
                           powiaty_dop,
                           " w odniesieniu do przeciętnego miesięcznego wynagrodzenia brutto w poszczególnych zamieszkiwanych przez nich powiatach, od września do grudnia ",
                           params$rok,
                           " roku - pracujący i niekontynuujący nauki"),
...
if (szk$l_abs[[1]] >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Odsetek absolwentów Państwa szkoły oraz pozostałych szkół policealnych z ",
                           powiaty_dop,
...
# 6. Aneks: wyniki dla absolwentów szkół policealnych z całego kraju
...
W aneksie zaprezentowano tabelaryczne rozkłady wybranych wskaźników przedstawionych w raporcie, ale dla ogółu absolwentów, którzy w roku szkolnym `r rocznik` ukończyli szkoły policealne w całym kraju..
...
god2$S3_12[[1]] %>%
  ...
  mutate(across(n, ~paste0("Absolwenci szkół policealnych\nw całym  kraju"))) %>%
  mutate(across(tylko_ucz:neet,
                ~paste0(round(. * 100), "\\%"))) %>%
  kable(format = "latex", booktabs = TRUE, escape = FALSE, longtable = TRUE,
        linesep = "",
...
if (nrow(tabela) > 0) {
  ...
  tabela %>% 
    ...
    kable_styling(position = wyrownanieTabWykr,
                  ...
                  full_width = TRUE,
                  repeat_header_text = "(ciąg dalszy tabeli)") %>% 
    add_header_above(c("Zawody, w których obecni absolwenci\nuczyli się w szkołach policealnych w całym kraju:", "1" = 1, "2" = 1, "3" = 1, "4" = 1),
                     align = c("l", "c", "c", "c", "c")) %>%
    add_header_above(c(" ",
                       "1. Tylko nauka\n2. Nauka i praca\n3. Tylko praca\n4. Brak nauki i brak pracy" = 4),
                     align = c("c", "l")) %>% 
  ...
  cat("W żadnym zawodzie, w którym absolwenci ukończyli kształcenie w Państwa szkole, na poziomie wszystkich szkół policealnych nie udało się zebrać minimum 10 absolwentów, aby móc przedstawić wyniki.")
}
```
...
tabelka %>% 
  ...
  kable_styling(position = wyrownanieTabWykr,
                ...
                full_width = FALSE,
                repeat_header_text = "(ciąg dalszy tabeli)") %>% 
  add_header_above(c("Sposób kontynuowania kształcenia", "Udział kontynuujących kształcenie wśród\nabsolwentów szkół policealnych z całego kraju" = 1),
                   align = c("l", "c"))
```

Odsetki mogą sumować się do więcej niż 100% ze względu na możliwość kontynuowania edukacji w więcej niż jednej formie.

3. Praca po ukończeniu szkoły policealnej w okresie od września do grudnia `r params$rok` roku

Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów szkół policealnych z całego kraju.
...
cbind(ucz, n_ucz[,2]) %>% 
  ...
  kable_styling(position = wyrownanieTabWykr,
                ...
                full_width = FALSE,
                repeat_header_text = "(ciąg dalszy tabeli)") %>%
  add_header_above(c(" ", "Absolwenci szkół policealnych z całego\nkraju kontynuujący naukę" = 1, "Absolwenci szkół policealnych z całego\nkraju niekontynuujący nauki" = 1),
                   align = c("l", "c", "c"))
...
Procentowy udział grup wyróżnionych ze względu na formę wykonywania pracy wśród pracujących absolwentów szkół policealnych z całego kraju.
...
cbind(ucz, n_ucz[,2]) %>% 
  ...
  kable("latex", booktabs = TRUE, escape = FALSE, longtable = TRUE,
    ...
    kable_styling(position = wyrownanieTabWykr,
                  ...
                  full_width = FALSE,
                  repeat_header_text = "(ciąg dalszy tabeli)") %>%
    add_header_above(c(" ", "Absolwenci szkół policealnych z całego\nkraju kontynuujących naukę" = 1, "Absolwenci szkół policealnych z całego\nkraju niekontynuujący nauki" = 1),
                   align = c("l", "c", "c"))
```
...
Procentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli zarejestrowani jako bezrobotni, wśród absolwentów szkół policealnych z całego kraju.
...
Procentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli bierni edukacyjnie i zawodowo, wśród absolwentów szkół policealnych z całego kraju.
