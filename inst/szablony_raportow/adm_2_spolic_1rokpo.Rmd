...
cat(
  ...
  params$rok,
  " roku (według stanu na 31 grudnia ", params$rok, " roku).\n\n* Dane pozyskane z Zakładu Ubezpieczeń Społecznych obejmują okres od 1 stycznia ", params$rok, " roku do 10 maja ROK roku. Informują o rejestrowanej przez ZUS aktywności zawodowej ",
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    params$rok,
    " roku kształciło się ",
    round((szk$S3_12[[1]]$ucz_prac + szk$S3_12[[1]]$ucz_prac) * 100),
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  ...
  wykres_poziomy_statusy(tab,
                       wrapper(paste0("Wykres ",
                                      wyk_num(),
                                      ": Status edukacyjno-zawodowy absolwentów Państwa szkoły oraz pozostałych szkół policealnych w grupie odniesienia, w grudniu ",
                                      params$rok,
                                      " roku - ogółem"), 80))
}
```
...
if (szk$Z4_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów Państwa szkoły oraz pozostałych szkół policealnych w ",
                           odmiany$god_dop,
...
if (szk$Z8_formy_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  cat(
    "W przypadku pracujących absolwentów Państwa szkoły, którzy nie kontynuowali nauki na studiach ani w szkole policealnej, w kolejnej szkole ponadpodstawowej, w grudniu ",
    params$rok,
    " roku dla ",
    ...
    " nie będący uczniami ani studentami absolwenci pozostałych szkół policealnych z ",
    odmiany$god_dop,
...
if (szk$W3_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Średni miesięczny przychód z pracy absolwentów Państwa szkoły oraz pozostałych szkół policealnych w ",
                           odmiany$god_dop,
...
if (szk$l_abs[[1]] >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ",
                           wyk_num(),
                           ": Odsetek absolwentów Państwa szkoły oraz pozostałych szkół policealnych w ",
                           odmiany$god_dop,
...
W aneksie zaprezentowano tabelaryczne rozkłady wskaźników przedstawionych w raporcie, ale dla ogółu szkół policealnych w całym kraju.

1. Rozkład statusów ogółem

Poniższa tabela przedstawia rozkład statusów w szkołach policealnych w całym kraju.

```{r, fig.align='left'}
kraj2$S3_12[[1]] %>%
  as_tibble() %>% 
  select(n, tylko_ucz, ucz_prac, tylko_prac, neet) %>% 
  mutate(across(n, ~paste0("Szkoły policealne w kraju"))) %>%
...
if (nrow(tabela) > 0) {
  ...
  cat("W żadnym zawodzie, w którym absolwentami zostali absolwenci Państwa szkoły, na poziomie wszystkich szkołach policealnych, nie udało się zebrać minimum 10 absolwentóW, aby móc pokazać wyniki.")
}
```

3. Sposoby kontynuowania kształcenia w grudniu `r params$rok` roku

Poniższa tabela przedstawia rozkład form kontynuacji kształcenia w szkołach policealnych w całym kraju. Odsetki mogą sumować się do więcej niż 100% ze względu na możliwość kontynuowania edukacji w więcej niż jednej formie.
...
tabelka %>% 
  ...
  kable_styling(position = wyrownanieTabWykr,
                ...
                full_width = FALSE,
                repeat_header_text = "(ciąg dalszy tabeli)") %>% 
  add_header_above(c(" ", "Sposoby kontynuowania kształcenia w\nszkołach policealnych w kraju" = 1),
                   align = c("l", "c"))
```

4. Praca po ukończeniu szkoły policealnej

Odsetek miesięcy, w których absolwenci szkół policealnych w całym kraju pracowali od września do grudnia `r params$rok` roku
...
Formy wykonywania pracy wśród absolwentów w szkołach policealnych w całym kraju od września do grudnia `r params$rok` roku
...
Rozkład liczby miesięcy bezrobocia wśród absolwentów w szkołach policealnych w całym kraju od września do grudnia `r params$rok` roku
...
7. Bierność edukacyjna i zawodowa

Rozkład liczby miesięcy bierności edukacyjnej i zawodowej wśród absolwentów w szkołach policealnych w całym kraju od września do grudnia `r params$rok` roku
