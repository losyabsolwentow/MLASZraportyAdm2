...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  ...
  wykres_poziomy_statusy(
    ...
    wrapper(paste0("Wykres ",
                   wyk_num(),
                   ": Status edukacyjno-zawodowy absolwentów Państwa szkoły oraz pozostałych techników w grupie odniesienia, w grudniu ",
                   params$rok,
                   " roku - ogółem"), 80))
}
```
...
W aneksie zaprezentowano tabelaryczne rozkłady wskaźników przedstawionych w raporcie, ale dla ogółu techników w całym kraju.

1. Rozkład statusów ogółem

Poniższa tabela przedstawia rozkład statusów w technikach w całym kraju.

```{r, fig.align='left'}
kraj2$S3_12[[1]] %>%
  as_tibble() %>% 
  select(n, tylko_ucz, ucz_prac, tylko_prac, neet) %>% 
  mutate(across(n, ~paste0("Technika w kraju"))) %>%
...
if (nrow(tabela) > 0) {
  ...
  cat("W żadnym zawodzie, w którym absolwentami zostali absolwenci Państwa szkoły, na poziomie wszystkich techników, nie udało się zebrać minimum 10 absolwentóW, aby móc pokazać wyniki.")
}
```

3. Sposoby kontynuowania kształcenia w grudniu `r params$rok` roku

Poniższa tabela przedstawia rozkład form kontynuacji kształcenia w technikach w całym kraju. Odsetki mogą sumować się do więcej niż 100% ze względu na możliwość kontynuowania edukacji w więcej niż jednej formie.
...
tabelka %>% 
  ...
  kable_styling(position = wyrownanieTabWykr,
                ...
                full_width = FALSE,
                repeat_header_text = "(ciąg dalszy tabeli)") %>% 
  add_header_above(c(" ", "Sposoby kontynuowania kształcenia w\ntechnikach w kraju" = 1),
                   align = c("l", "c"))
```

4. Praca po ukończeniu technikum

Odsetek miesięcy, w których absolwenci techników w całym kraju pracowali od września do grudnia `r params$rok` roku
...
Formy wykonywania pracy wśród absolwentów w technikach w całym kraju od września do grudnia `r params$rok` roku
...
Rozkład liczby miesięcy bezrobocia wśród absolwentów w technikach w całym kraju od września do grudnia `r params$rok` roku
...
Rozkład liczby miesięcy bierności edukacyjnej i zawodowej wśród absolwentów w technikach w całym kraju od września do grudnia `r params$rok` roku
