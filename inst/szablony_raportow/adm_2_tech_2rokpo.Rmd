...
Niektóre wyniki dotyczące absolwentów `r ifelse(szk$l_abs[[1]] >= progLiczebnosci, paste0("Państwa szkoły"), paste0("poszczególnych szkół"))` zostały zestawione z wynikami dotyczącymi absolwentów pozostałych techników z województwa^[Absolwenci Państwa szkoły są wykluczani z tej grupy porównawczej, ponieważ w niektórych przypadkach ich sytuacja mogłaby w dużym stopniu rzutować na wyniki dotyczące tej grupy.] `r szk$formy[[1]]$woj_dop` oraz z całego kraju.
...
if (szk$S3_0_12[[1]]$n >= progLiczebnosci) {
  ...
  levels(tab$typ)[grepl("^Pozostałe", levels(tab$typ))] = paste0("Pozostałe technika\nz województwa\n", szk$formy[[1]]$woj_dop)
  levels(tab$typ)[grepl("^Branżowe szkoły", levels(tab$typ))] = "Technika w całej Polsce"
  
  
  wykres_poziomy_1_ad1(tab,
                       paste0("Wykres ", wyk_num(), ": Status edukacyjno-zawodowy absolwentów Państwa szkoły oraz pozostałych\ntechników w województwie i w kraju, w grudniu 2020 roku - ogółem"))
}
```
...
if (szk$Z4_ucz[[1]]$n >= progLiczebnosci) {
  ...
  levels(tab_wyk$typ)[grepl("^Pozostałe", levels(tab_wyk$typ))] = paste0("Pozostałe technika\nz województwa\n", szk$formy[[1]]$woj_dop)
  levels(tab_wyk$typ)[grepl("^Branżowe szkoły", levels(tab_wyk$typ))] = "Technika w całej Polsce"
  
  tab_wyk$name = ordered(tab_wyk$name, levels = c("brak\npracy", "część\nokresu", "cały\nokres"))
  
  wyk_tyt = wrapper(paste0("Wykres ", wyk_num(), ": Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów Państwa szkoły oraz pozostałych techników w województwie i w kraju, od września do grudnia 2020 roku - studiujący lub uczący się w szkołach policealnych."),
           width = 85)
...
if (szk$Z4_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ", wyk_num(), ": Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów Państwa szkoły oraz pozostałych techników w województwie i w kraju, września do grudnia 2020 - nie studiujący ani nie uczący się w szkołach policealnych"),
           width = 85)
...
if (szk$Z8_formy_ucz[[1]]$n >= progLiczebnosci) {
  ...
  etykiety = c("Tylko umowa o pracę",
               "Tylko samozatrudnienie",
  ...
  wyk_tyt = wrapper(paste0("Wykres ", wyk_num(), ": Formy wykonywania pracy przez absolwentów Państwa szkoły oraz pozostałych techników w województwie i w kraju, w grudniu 2020 roku - pracujący i uczący się (%)"),
           width = 85)
...
if (szk$Z8_formy_nie_ucz[[1]]$n >= progLiczebnosci) {
  ...
  cat(
    "Natomiast w przypadku pracujących absolwentów Państwa szkoły, którzy nie kontynuowali nauki na studiach, w szkole policealnej ani w kolejnej szkole ponadpodstawowej, w grudniu 2020 roku dla ",
    ...
    " nie będący uczniami ani studentami absolwenci pozostałych techników z województwa ",
    szk$formy[[1]]$woj_dop,
  ...
  wyk_tyt = wrapper(paste0("Wykres ", wyk_num(), ": Formy wykonywania pracy przez absolwentów Państwa szkoły oraz pozostałych techników w województwie i w kraju, w grudniu 2020 roku - pracujący i nie uczący się (%)"),
           width = 85)
...
if (szk$W1_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    " zł. To znaczy, że co najmniej połowa grupy, dla której dostępne były informacje o zarobkach, uzyskiwała średni miesięczny przychód nieprzekraczający tej kwoty, a co najmniej połowa - przychód w wysokosci co najmniej tej kwoty. W przypadku absolwentów pozostałych techników z województwa ",
    szk$formy[[1]]$woj_dop,
  ...
  wyk_tyt = wrapper(paste0("Wykres ", wyk_num(), ": Miesięczne przychody brutto z pracy absolwentów Państwa szkoły oraz pozostałych techników w województwie i w kraju, od września do grudnia 2020 roku - pracujący i uczący się na studiach lub w szkole policealnej"),
           width = 85)
  
  wykres = boxplot_zl_4_ad1(x = tab_wyk,
                tytul = wyk_tyt) +
...
if (szk$W1_nie_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    " zł. To znaczy, że co najmniej połowa grupy, dla której dostępne były informacje o zarobkach, uzyskiwała średni miesięczny przychód nieprzekraczający tej kwoty, a co najmniej połowa - przychód w wysokości co najmniej tej kwoty. W przypadku absolwentów pozostałych techników z województwa ",
    szk$formy[[1]]$woj_dop,
  ...
  wyk_tyt = wrapper(paste0("Wykres ", wyk_num(), ": Miesięczne przychody brutto absolwentów Państwa szkoły oraz pozostałych techników w województwie i w kraju, od września do grudnia 2020 roku - pracujący i nie studiujący ani nie uczący się w szkole policealnej"),
           width = 85)
...
if (szk$W3_ucz[[1]]$n >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ", wyk_num(), ": Średni miesięczny przychód z pracy absolwentów Państwa szkoły oraz pozostałych techników w województwie i w kraju z pracy etatowej w odniesieniu do średnich zarobków w poszczególnych zamieszkiwanych przez nich powiatach, od września do grudnia 2020 roku - pracujący i studiujący lub uczący się w szkole policealnej"),
           width = 85)
...
if (szk$W3_nie_ucz[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% przeciętnych wynagrodzeń w powiatach, w których mieszkali. Dla pracujących na etat i nieuczących się absolwentów pozostałych techników z województwa ",
    szk$formy[[1]]$woj_dop,
     " wartość mediany wynosiła ",
  ...
  wyk_tyt = wrapper(paste0("Wykres ", wyk_num(), ": Średni miesięczny przychód z pracy absolwentów Państwa szkoły oraz pozostałych techników w województwie i w kraju z pracy etatowej w odniesieniu do średnich zarobków w poszczególnych zamieszkiwanych przez nich powiatach, od września do grudnia 2020 roku - pracujący i nie studiujący ani nie uczący się w szkole policealnej"),
           width = 85)
...
if (szk$l_abs[[1]] >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ", wyk_num(), ": Odsetek absolwentów Państwa szkoły oraz pozostałych techników w województwie i w kraju, którzy byli bezrobotni przez daną liczbę miesięcy, od września do grudnia 2020 roku"),
           width = 85)
...
if (szk$l_abs[[1]] >= progLiczebnosci) {
  ...
  wyk_tyt = wrapper(paste0("Wykres ", wyk_num(), ": Odsetek absolwentów Państwa szkoły oraz pozostałych techników w województwie i w kraju, którzy byli bierni edukacyjnie i zawodowo przez daną liczbę miesięcy, od września do grudnia 2020 roku"),
           width = 85)
